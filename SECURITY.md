...
If you believe you have found a security vulnerability in `z4j-fastapi`,
...
This integration starts the agent inside FastAPI processes, reads agent
secrets, accepts native engine handles, and composes application lifespan
callbacks. Secret handling, handle selection, startup/shutdown failure
isolation, and lifespan composition are package-specific security surfaces;
transport, payload redaction, and authorization policy remain owned by
`z4j-core` and the brain.
